...
def ingest_doc_to_cosmos(doc_path: DocPath , file) -> None:
    ...
    for i in range(0, num_chunks, batch_size):
        ...
        documents = [{"id": f"{file_name}-{i}", "content": text, "metadata": metadata , "file_name" : file.filename } for i, text in enumerate(batch_texts)]
    ...
    for link in link_list:
        try:
            ...
            for i in range(0, num_chunks, batch_size):
                ...
                documents = [{"id": f"{encoded_link}-{i}", "content": text, "metadata": metadata , "file_name" : link} for i, text in enumerate(batch_texts)]
